...
@app.route("/")
def home():
    """Smart landing: once a roster has actually been saved on the
    Season side (any team's, not just this visitor's), that's presumably
    the more useful default for the rest of the year - jump straight
    there. Before that (pre-draft, or a fresh deploy with no season data
    yet), land on the Draft Room instead."""
    league = season.load_league()
    has_roster = any(team.get("roster") for team in league["teams"].values())
    return redirect(url_for("season_page" if has_roster else "draft_room"))
...
@app.route("/api/season/import_from_draft", methods=["POST"])
def season_import_from_draft():
    """Copies the current draft's rosters - and its league size/starter
    settings - into the shared Season store. The natural bridge once a
    draft wraps: nobody should have to retype eight rosters by hand
    into a textarea when DraftIQ already has them. Safe to run more
    than once (e.g. to pick up late roster moves before the season
    starts) - it just overwrites Season's team records with the
    draft's current state."""
    engine = get_engine()
    if not engine.loaded:
        return jsonify({"ok": False, "message": "No draft loaded - nothing to import."}), 400
    if not engine.league:
        return jsonify({"ok": False, "message": "Draft hasn't started yet - nothing to import."}), 400

    season.update_league_settings(num_teams=engine.num_teams, starters=dict(engine.starters))
    for i in range(1, engine.num_teams + 1):
        team_name = engine.league[i - 1]["team"]
        roster = engine.get_team_roster(i)
        season.update_team_roster(i, name=team_name, roster=roster)

    # Carry over which team is "mine" too, so the Season page opens
    # already pointed at the right team for this visitor.
    session["season_my_team"] = engine.my_team

    return jsonify({"ok": True, "num_teams": engine.num_teams, "my_team": engine.my_team})
